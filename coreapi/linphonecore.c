...
void linphone_core_set_preview_corner(LinphoneCore *lc, int corner) {
    if (corner!=lc->preview_corner) ms_message("%s : corner=%d\n", __FUNCTION__, corner);
    lc->preview_corner = corner;
#ifdef VIDEO_ENABLED
    {
        LinphoneCall *call=linphone_core_get_current_call(lc);
        if (call!=NULL && call->videostream){
            video_stream_set_preview_corner(call->videostream,corner);
        }
    }
#endif
}
