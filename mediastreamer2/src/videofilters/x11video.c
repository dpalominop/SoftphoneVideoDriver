...
static void x11video_fill_background(MSFilter *f){
    ...
    uint8_t rgb[3]={0,0,0};/*0 en RGB*/
    uint8_t yuv[3]={0,0,0};/*0 en YUV*/
	int ysize=s->fbuf.h*s->fbuf.strides[0];
	int usize=s->fbuf.h*s->fbuf.strides[1]/2;
	int vsize=s->fbuf.h*s->fbuf.strides[2]/2;
    ms_rgb_to_yuv(rgb,yuv);/*convierte 0 en RGB a YUV*/
    /*Llena con 0 en RGB o color negro a todos los pixeles en YUV */
...
static void x11video_prepare(MSFilter *f){
    ...
    //ms_message("xai->name: *****, xai->num_adaptors: %lu, xai->num_formats: %lu, xai->num_ports: %lu, xai->type: %c",xai->num_adaptors,xai->num_formats,xai->num_ports, xai->type);
    XSetErrorHandler(x11error_handler);ms_message("###################nadaptors: %i\n",nadaptors);
...
static void x11video_process(MSFilter *f){
	...
	if (f->inputs[0]!=NULL && (inm=ms_queue_peek_last(f->inputs[0]))!=0) {
        if (ms_yuv_buf_init_from_mblk(&src,inm)==0)/*se usa un frame para formatear a src*/{
        //if (ms_yuv_buf_init_from_mblk_with_size(&src,inm, 240, 320)==0)/*se usa un frame para formatear a src*/{

            if(obj->rotate==270) ms_yuv_buf_init_from_mblk_with_size(&src,inm, src.h, src.w);

			MSVideoSize newsize;
            newsize.width=src.w;//ancho de frame
            newsize.height=src.h;//alto de frame
			precious=mblk_get_precious_flag(inm);

            if (!ms_video_size_equal(newsize,obj->vsize) )/*lo normal es que las dimensiones del video sean diferentes a las de la ventana*/ {
				ms_message("received size is %ix%i",newsize.width,newsize.height);
                obj->vsize=newsize;//se cargan las dimensiones extraidas del frame
				if (obj->autofit){
    ...
    /*Se calcula el rectangulo principal y local centrados en la ventana*/
	...
	if (lsrc.w!=0 && obj->corner!=-1){
		...
		if (obj->local_msg==NULL){
            /*Se formatea la estructura MSPicture: obj->local_pic con el valor de ancho y largo de localrect*/
	...
	if (update && src.w!=0){
        ...
        if (obj->rotate==270) ms_yuv_buf_rotate(&obj->fbuf);
	...
	if (update && obj->local_msg!=NULL && obj->corner!=-1){
        /*se copia el struct del video entrante (obj->fbuf) a corner*/
        MSPicture corner=obj->fbuf;
        /*se copia el tamaño del video local a roi*/
		MSVideoSize roi;
        roi.width=obj->local_pic.w;
        roi.height=obj->local_pic.h;
        /*se copia el tamaño del video local a corner*/
        corner.w=obj->local_pic.w;
        corner.h=obj->local_pic.h;

        ms_layout_set_preview_pos(obj->vsize, roi, &localrect, obj->rotate, obj->corner);

        /*se llevan los punteros de los planos al lugar donde se quiere insertar el video local*/
        ...
        /*se copia el video local(local_pic) dentro del espacio separado anteriormente(corner) en el video externo*/

        if(obj->rotate==270){
            ms_yuv_buf_copy_rotated(obj->local_pic.planes,obj->local_pic.strides,
	...
	if (update){
        /*obtiene el rectangulo centrado en funcion del tamaño de la ventana y del tamaño del video*/
        MSRect rect;
        ms_layout_center_rectangle(obj->wsize,obj->vsize,&rect);

        /*Se dibuja el video fusionado en el rectangulo creado anteriormente*/
...
static int x11video_set_rotation(MSFilter *f,void *arg){
    X11Video *s=(X11Video*)f->data;
    s->rotate=*(int*)arg;
    return 0;
}
...
/* methods for compatibility with the MSVideoDisplay interface*/
    ...
    {   MS_VIDEO_DISPLAY_SET_DEVICE_ORIENTATION, x11video_set_rotation},
	{	0	,NULL}
};
