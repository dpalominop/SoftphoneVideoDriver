...
void ms_layout_compute(MSVideoSize wsize, MSVideoSize vsize, MSVideoSize orig_psize,  int localrect_pos, float scalefactor, MSRect *mainrect, MSRect *localrect){
	...
	if (localrect_pos!=-1){
		...
		if ((wsize.height - mainrect->h < mainrect->h/scalefactor && wsize.width - mainrect->w < mainrect->w/scalefactor) || localrect_pos<=3)
		{
			int x_sv;
			int y_sv;
            if (localrect_pos%4==1) /* corner = 1 */
			{
				/* top left corner */
				x_sv = LOCAL_POS_OFFSET;
				y_sv = LOCAL_POS_OFFSET;
			}
            else if (localrect_pos%4==2) /* corner = 2 */
			{
				/* top right corner */
				x_sv = (wsize.width-localrect->w-LOCAL_POS_OFFSET);
				y_sv = LOCAL_POS_OFFSET;
			}
            else if (localrect_pos%4==3) /* corner = 3 */
			{
				/* bottom left corner */
				x_sv = LOCAL_POS_OFFSET;
...
void ms_layout_set_preview_pos(MSVideoSize wsize, MSVideoSize lsize, MSRect *rect, int orientation, int corner){
    int x_sv;
    int y_sv;
    if (corner%4==1) /* corner = 1 */
    {
        /* top left corner */
        x_sv = LOCAL_POS_OFFSET;
        y_sv = LOCAL_POS_OFFSET;
    }
    else if (corner%4==2) /* corner = 2 */
    {
        /* top right corner */
        if(orientation==270){
            x_sv = (wsize.width-lsize.height-LOCAL_POS_OFFSET);
        }else{
            x_sv = (wsize.width-lsize.width-LOCAL_POS_OFFSET);
        }
        y_sv = LOCAL_POS_OFFSET;
    }
    else if (corner%4==3) /* corner = 3 */
    {
        /* bottom left corner */
        x_sv = LOCAL_POS_OFFSET;
        if(orientation==270){
            y_sv = (wsize.height-lsize.width-LOCAL_POS_OFFSET);
        }else{
            y_sv = (wsize.height-lsize.height-LOCAL_POS_OFFSET);
        }
    }
    else /* corner = 0: default */
    {
