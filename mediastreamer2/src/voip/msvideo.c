...
static void plane_rotate(uint8_t *p, int linesize, int w, int h, uint8_t *tmp_image){

    int i,j;
    if(linesize==w){
        memcpy(tmp_image, p, h*w*sizeof(uint8_t));
        for(j=0;j<w;j++){
            for(i=0;i<h;i++){
                p[(h-1-i)*linesize+j]=tmp_image[i+j*h];
            }
        }
    }else{
        uint8_t *pp = p;
        uint8_t *tt = tmp_image;
        for(j=0;j<h;j++){
            memcpy(tt,pp,w);
            pp+=linesize;
            tt+=w;
...
/*rotate*/
void ms_yuv_buf_rotate(YuvBuf *buf){
    uint8_t *tmp_image=(uint8_t *)malloc(buf->w*buf->h*sizeof(uint8_t));

    plane_rotate(buf->planes[0],buf->strides[0],buf->w,buf->h, tmp_image);
    plane_rotate(buf->planes[1],buf->strides[1],buf->w/2,buf->h/2, tmp_image);
    plane_rotate(buf->planes[2],buf->strides[2],buf->w/2,buf->h/2, tmp_image);

    free(tmp_image);
}

static void plane_copy_rotated(const uint8_t *src_plane, int src_stride,
    uint8_t *dst_plane, int dst_stride, MSVideoSize roi){

    int i,j;
    for(j=0;j<roi.height;j++){
        for(i=0;i<roi.width;i++){
            dst_plane[(roi.width-1-i)*dst_stride+j]=src_plane[i+j*roi.width];
        }
    }

}

void ms_yuv_buf_copy_rotated(uint8_t *src_planes[], const int src_strides[],
        uint8_t *dst_planes[], const int dst_strides[3], MSVideoSize roi){
    plane_copy_rotated(src_planes[0],src_strides[0],dst_planes[0],dst_strides[0],roi);
    roi.width=roi.width/2;
    roi.height=roi.height/2;
    plane_copy_rotated(src_planes[1],src_strides[1],dst_planes[1],dst_strides[1],roi);
    plane_copy_rotated(src_planes[2],src_strides[2],dst_planes[2],dst_strides[2],roi);
}
